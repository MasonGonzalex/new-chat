...
apiRouter.post("/chat-request", (req, res) => {
  ...
  (async () => {
    ...
    async function fetchWithGeminiFailover(provider, purifiedMessages) {
        ...
        // Start from the current index and try all keys once
        for (let i = 0; i < totalKeys; i++) {
            ...
            if (response.ok) {
                // Success! Update the index to start from the next key next time
                provider.currentKeyIndex = (keyIndex + 1) % totalKeys;
                return response;
            }
    
            // If not OK, log it and the loop will try the next key
            console.warn(`Gemini API key at index ${keyIndex} failed with status ${response.status}.`);
        }
